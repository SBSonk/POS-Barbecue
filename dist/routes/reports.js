...
router.get('/eod', async (req, res) => {
    try {
      ...
      WHERE ${dateCondition}
    `;
        const totals = await db.get(totalsQuery, dateParams);
    ...
    `;
        const payments = await db.all(paymentsQuery, dateParams);
        let cashTotal = 0;
        let qrphTotal = 0;
        let cardTotal = 0;
        let transferTotal = 0;
    ...
    `;
        const productSales = await db.all(productSalesQuery, dateParams);
    ...
    `;
        const categorySales = await db.all(categorySalesQuery, dateParams);
      ...
      ORDER BY hour ASC
    `;
        const hourlySales = await db.all(hourlySalesQuery, dateParams);
        res.json({
            success: true,
