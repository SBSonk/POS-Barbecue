...
router.post('/verify-pin', async (req, res) => {
    try {
        ...
        const currentPinRow = await db.get("SELECT value FROM settings WHERE key = 'admin_pin'");
...
router.put('/', async (req, res) => {
    try {
        ...
        if (business_name !== undefined)
            await upsertSetting('business_name', String(business_name).trim());
        if (currency_symbol !== undefined)
            await upsertSetting('currency_symbol', String(currency_symbol).trim());
        if (tax_rate !== undefined)
            await upsertSetting('tax_rate', String(tax_rate).trim());
