...
function getDb() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const dbPath = process.env.DB_PATH || path_1.default.join(process.cwd(), 'pos.db');
...
function initDatabase() {
  ...
  `);
        ...
        yield db.run('INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)', ['business_name', 'Smokin\' BBQ & Brews']);
        yield db.run('INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)', ['currency_symbol', '$']);
        yield db.run('INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)', ['admin_pin', '1234']);
        yield db.run('INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)', ['tax_rate', '0']);
        return db;
    });
}
function seedInitialProducts() {
    return __awaiter(this, void 0, void 0, function* () {
        const db = yield getDb();
        const countRow = yield db.get('SELECT COUNT(*) as count FROM products');
