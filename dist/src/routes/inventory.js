...
router.get('/low-stock', (_req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
    ...
    `);
        const items = rows.map((prod) => (Object.assign(Object.assign({}, prod), { is_low_stock: prod.stock <= prod.low_stock_threshold && prod.stock > 0, is_out_of_stock: prod.stock <= 0 })));
...
router.post('/quick-restock', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const db = yield (0, db_1.getDb)();
        ...
        try {
            ...
            for (const item of updates) {
                ...
                if (isNaN(prodId) || isNaN(addStock) || addStock === 0)
                    continue;
                const prod = yield db.get('SELECT * FROM products WHERE id = ?', [prodId]);
                if (!prod)
                    continue;
                const newStock = Math.max(0, prod.stock + addStock);
                yield db.run('UPDATE products SET stock = ?, updated_at = DATETIME(\'now\', \'localtime\') WHERE id = ?', [newStock, prodId]);
                yield db.run(`
