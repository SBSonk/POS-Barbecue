...
router.get('/eod', async (req: Request, res: Response) => {
  try {
    ...
    const totalsQuery = `
      ...
      WHERE ${dateCondition}
    `;
    const totals = await db.get(totalsQuery, dateParams) as any;

    const completedOrders = totals?.completed_orders || 0;
    const pendingOrders = totals?.pending_orders || 0;
    ...
    const paymentsQuery = `
      ...
      FROM transactions t
      WHERE ${dateCondition} AND status IN ('COMPLETED', 'PENDING')
      GROUP BY payment_method
    `;
    const payments = await db.all(paymentsQuery, dateParams) as any[];
    ...
    for (const p of payments) {
      if (p.payment_method === 'CASH') cashTotal = p.total_amount;
      else if (p.payment_method === 'QRPH') qrphTotal = p.total_amount;
      else if (p.payment_method === 'CARD') cardTotal = p.total_amount;
      else if (p.payment_method === 'TRANSFER') transferTotal = p.total_amount;
    }
    ...
    const productSalesQuery = `
      ...
      WHERE ${dateCondition} AND t.status IN ('COMPLETED', 'PENDING')
      GROUP BY ti.product_id, ti.product_name, ti.category
      ORDER BY quantity_sold DESC, revenue_generated DESC
    `;
    const productSales = await db.all(productSalesQuery, dateParams) as any[];
    ...
    `;
    const categorySales = await db.all(categorySalesQuery, dateParams) as any[];
    ...
    const hourlySalesQuery = `
      ...
      ORDER BY hour ASC
    `;
    const hourlySales = await db.all(hourlySalesQuery, dateParams) as any[];
    ...
    res.json({
      ...
      summary: {
        ...
        voided_orders: totals?.voided_orders || 0,
        total_revenue: totalRevenue,
        pending_revenue: totals?.pending_revenue || 0,
        total_tax: totals?.total_tax || 0,
        total_discount: totals?.total_discount || 0,
        total_voided_amount: totals?.total_voided_amount || 0,
        avg_order_value: avgOrderValue,
        total_items_sold: totalItemsSold,
        cash_revenue: cashTotal,
        qrph_revenue: qrphTotal,
