  ...
  try {
    ...
    try {
      ...
      for (const item of items) {
        ...
        const product = await db.get('SELECT * FROM products WHERE id = ?', [prodId]) as any;
        if (!product) {
      ...
      const taxRateSetting = await db.get("SELECT value FROM settings WHERE key = 'tax_rate'") as any;
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const transactions = await db.all(query, params) as any[];
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const db = await getDb();
    const transaction = await db.get('SELECT * FROM transactions WHERE id = ?', [req.params.id]) as any;
    if (!transaction) {
...
router.post('/:id/void', async (req: Request, res: Response) => {
  try {
    ...
    const items = await db.all('SELECT * FROM transaction_items WHERE transaction_id = ?', [id]) as any[];
    ...
    try {
      ...
      for (const item of items) {
        ...
        const updatedProd = await db.get('SELECT stock FROM products WHERE id = ?', [item.product_id]) as any;
